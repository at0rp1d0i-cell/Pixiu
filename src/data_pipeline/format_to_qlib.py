...
def main():
    ...
    logger.info("找到 %d 个 Parquet 清洗文件。", len(parquet_files))
    
    # Step 1: 构建统一交易日历
    logger.info("Step 1/3: 聚合系统交易日历 (Calendars)...")
    all_dates = set()
    ...
    with open(os.path.join(cal_dir, f"{FREQ}.txt"), "w") as f:
        for d in calendar:
            f.write(d + "\n")
    logger.info("  日历边界: %d 个交易日 (%s ~ %s)", len(calendar), calendar[0], calendar[-1])
    
    # Step 2: 序列化浮点数因子至内存数组
    logger.info("Step 2/3: 执行浮点数量化序列化 (注入 start_index)...")
    ...
    # Step 3: 构建系统基准池索引
    logger.info("Step 3/3: 封装 Instruments 索引集...")
    ...
    logger.info("装载标的: %d 只成分股", len(stock_info))
    logger.info("平台数据隔离区重构完成！Qlib 引擎载入地址: %s", QLIB_DIR)
