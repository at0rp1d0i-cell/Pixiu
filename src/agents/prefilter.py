...
def _format_factor_algebra_family_context(note: FactorResearchNote) -> str:
    if note.exploration_subspace != ExplorationSubspace.FACTOR_ALGEBRA:
        return ""
    family_key, _ = NoveltyFilter._resolve_factor_gene_keys_from_note(note)
    if family_key is None:
        return ""

    recipe = NoveltyFilter._reverse_factor_algebra_formula_recipe(note.final_formula or note.proposed_formula)
    if recipe is None:
        return ""

    semantics = {
        "mean_spread": "表示长短窗口均值差，不是收益率变化或动量加速度。",
        "ratio_momentum": "表示长短窗口相对强弱/比值动量，不是均线差。",
        "volatility_state": "表示长短窗口波动状态差，不是价格动量或收益率变化。",
        "volume_confirmation": (
            "表示量价差值确认；canonical form 可以是 "
            "Mul(price_mean_spread, volume_mean_spread)。Mul 在当前运行时允许，"
            "这不是未批准算子，也不是 generic momentum。"
        ),
    }.get(recipe.transform_family, "")
    if not semantics:
        return ""
...
class AlignmentChecker:
    ...
    async def check(self, note: FactorResearchNote) -> tuple[bool, str]:
        formula = note.final_formula or note.proposed_formula
        family_context = _format_factor_algebra_family_context(note)
