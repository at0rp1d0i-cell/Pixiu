...
| `16_test-pipeline.md` | active | 默认 pytest 入口已可用；当前 smoke/unit 基线为 527 passed，CLI smoke 与 approval 注入口径已补；marker 显式化已完成，但全局状态隔离和 contract test 仍在 Test Pipeline Refactor 中收口 |
...
- canonical test entry 已存在，CLI / approval 的 smoke 路径已补齐；marker 体系已完成显式收口，但全局状态隔离和真实链路仍需 live / e2e 校验
...
- `live / e2e / CI` 还没有完全收口
- marker 显式化已完成，但 `sys.path` / 全局状态隔离和 live 测试环境仍需要继续收口
