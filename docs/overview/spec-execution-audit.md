...
| `stage-2-hypothesis-expansion.md` | implemented/partial | 新设计已明确五个探索子空间与目标对象，但运行时仍主要停留在 note generation |
| `stage-3-prefilter.md` | implemented/partial | 主过滤链已实现并有测试 |
| `stage-4-execution.md` | implemented/partial | 已收敛到 `src/execution/` 主路径，report contract 仍未完全收口 |
| `stage-5-judgment.md` | implemented/partial | canonical runtime 已收敛到 `src/agents/judgment.py`，`critic.py / factor_pool_writer.py / cio_report_renderer.py` 仅保留兼容职责 |
| `stage-45-golden-path.md` | active | 当前 Stage 4→5 收口入口 |
| `factor-pool.md` | implemented/partial | richer metadata 写回已开始，独立记录模型仍待继续收口 |
...
2. 将 Stage 2 明确升级为 `Hypothesis Expansion Engine`。
3. 继续补齐 `BacktestReport / CriticVerdict / FactorPoolRecord` richer contract。
4. 扩展控制平面，直到 CLI/API 的数据面稳定。
5. 最后再推进 Dashboard、数据源扩展和真实环境闭环。
