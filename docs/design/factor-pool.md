...
# 每个因子文档的字段
{
    ...
    "metadata": {
        "island": str,
        "formula": str,
        "hypothesis": str,     # 新增：经济直觉
        "passed": bool,
        "sharpe": float,
        "ic_mean": float,
        "icir": float,
        "turnover_rate": float,
        "failure_mode": str,   # 若失败，记录失败类型
        "overfitting_score": float,  # 新增：RiskAuditor 输出
        "date": str,
        "iteration": int,
        "tags": str,           # JSON 序列化的 List[str]，ChromaDB 不支持 list
    }
}
```
