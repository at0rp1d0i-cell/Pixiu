...
- `interface-contracts.md` 必须映射到 schema 测试。
- `stage-3-prefilter.md` 必须映射到 prefilter 测试。
- `stage-4-execution.md` 必须映射到 execution 测试。
- `stage-5-judgment.md` 必须映射到 judgment 测试。
- `terminal-dashboard.md` 的 CLI/API 最小实现，必须至少有 import 和接口 smoke 测试。
