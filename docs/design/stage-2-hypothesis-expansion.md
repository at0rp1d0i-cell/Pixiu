...
| 缺口 | Phase 2 子任务 | 状态 |
|------|--------------|------|
| Exploration subspace registry | 2.3 Factor Algebra 原语注册表 | 待实现 |
| Mutation operator vocabulary | 2.4 Symbolic Mutation 运行时 | 待实现 |
| `Hypothesis / StrategySpec` 正式 schema | **已完成**（Phase 1） | ✅ |
| FailureConstraint 结构化沉淀 | 2.1（参见 `failure-constraint.md`） | 待实现 |
| SynthesisAgent 功能实现 | 2.2（参见 `synthesis-agent.md`） | 待实现 |
| Regime Detection 标准化 | 2.5（参见 §11） | 待实现 |
...
| 主题 | 文档 |
|------|------|
| Phase 2 整体执行计划 | `docs/plans/phase-2-hypothesis-engine.md` |
| FailureConstraint 设计 | `docs/design/failure-constraint.md` |
| SynthesisAgent 设计 | `docs/design/synthesis-agent.md` |
| Factor Algebra 原语注册表 | 本文 §9 |
| Symbolic Mutation 运行时 | 本文 §10 |
| Regime Detection 模块 | 本文 §11 |
| 当前实现偏差 | `docs/overview/spec-execution-audit.md` |
