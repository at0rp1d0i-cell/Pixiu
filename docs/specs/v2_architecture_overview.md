...
> 版本：2.0
> 创建：2026-03-07
> 优先级：**P0 — 所有其他 Spec 的前置依赖，必须先读此文档**

---

## 1. 设计哲学：为 Agent 设计，而非模仿人类

### v1 的问题

v1 是线性管线（Researcher → Validator → Coder → Critic），本质上在用 AI 模仿人类研究员的工作方式——一个研究员、一次一个方向、顺序等待回测。这导致：
...
**Pixiu 的出发点不是「如何让 AI 替代人类研究员」，而是「如果从零开始为 Agent 设计量化研究流程，它应该是什么样的」。**

人类量化团队受到的约束，Agent 不受：

| 人类团队的约束 | Pixiu 的设计选择 |
|---|---|
| 研究员同时只能研究一个方向 | 6 个 AlphaResearcher 并行，成本线性 |
| 研究员会「失忆」，不记得失败案例 | FactorPool 永久存储所有失败，检索驱动下一轮 |
| 研究员有职业风险偏差，不敢提「奇怪」假设 | Agent 无职业焦虑，探索边界更广 |
| 反馈循环需要数天（开会、对齐、编码、等回测） | 分钟级自动循环 |
| 每次生成 1 份研报 | 每轮生成 12-18 个候选，漏斗式筛选 |
...
**与同类项目的关键差异**：
- **vs RD-Agent（Microsoft）**：RD-Agent 用文档驱动接口，但 Azure 锁定、无 A 股特化、无漏斗过滤。Pixiu 在此基础上增加并行宽度和前置过滤层。
- **vs TradingAgents**：TradingAgents 照搬人类组织结构（Analyst/Researcher/RiskManager），Agent 之间模拟「开会辩论」，是把人类通信成本搬进系统。Pixiu 的架构不参照人类团队，而是按 LLM 的实际优势（并行、无疲劳、永久记忆）重新设计。
...
完整 Agent 清单与职责边界已拆分到 `v2_agent_team.md`。

在架构总览层，只保留三条核心原则：

1. Pixiu 不是“所有节点都用 LLM”的系统，确定性组件和 LLM 组件必须共存。
2. `Coder`、`Validator`、`NoveltyFilter` 这类组件必须优先保持确定性。
3. `ReflectionAgent` 属于跨轮学习层，不在五阶段主漏斗内。
...
### 开发断点（Development Breakpoints）

系统运行期间设置 6 个断点，供人工审核或分析。每个断点自动输出 Markdown 评估报告。

| 断点 | 时机 | 审核内容 | 决策 |
|---|---|---|---|
| BP-1 | Day 1 结束 | 反思库初始质量 | Continue / Adjust Prompt |
| BP-2 | Day 5，50+ 因子 | 失败分布合理性 | Continue / Reset Island |
| BP-3 | Day 8，模拟盘 7 天 | OOS 退化趋势 | Continue / 暂停 |
| BP-4 | Day 21，OOS 完成 | Sharpe 退化率审查 | 继续纸测 / 重新调参 |
| BP-5 | Month 2，反思库稳定 | ReflectionAgent 的 Skill 提案 | 审批永久 Skills |
| BP-6 | Month 3，纸测 6 周 | 实盘准备就绪评估 | 批准小额实盘 |
...
11. `v2_data_sources.md` — 数据层扩展（AKShare 补充 + Tushare 接入）
...
反思系统已拆分到 `v2_reflection_system.md`。

在架构总览层，只保留三条核心原则：

1. FactorPool 只记录结果，ReflectionSystem 负责沉淀过程知识。
2. 反思分为动态反思、元反思、永久 Skills 三层生命周期。
3. 反思系统属于跨轮异步能力，不应挤进五阶段主漏斗。

---

## 10. OOS 与泛化验证（摘要）

OOS 与泛化验证已拆分到 `v2_oos_and_generalization.md`。

在架构总览层，只保留三条核心原则：

1. Pixiu 不只防价格数据 look-ahead bias，也防 LLM 的知识污染偏差。
2. 因子从发现之日起就要绑定 OOS 时间边界。
3. OOS 是进入正式组合前的验证层，而不是可选附加项。

---

## 11. 系统启动设计（摘要）

系统启动与冷启动节奏已拆分到 `v2_system_bootstrap.md`。

在架构总览层，只保留两条核心原则：

1. 冷启动的瓶颈是市场日历，不是系统吞吐。
2. 启动期必须设计人工断点，避免在早期把错误机制快速放大。

---

## 12. 商业化架构原则（摘要）

商业化原则已拆分到 `v2_commercialization_principles.md`。

在架构总览层，只保留两条核心原则：

1. 可开源的是引擎和公共约束，不可开源的是因子库与反思资产。
2. 商业化原则只能影响架构边界，不能反向污染当前 Stage 4/5 的基础工程优先级。
