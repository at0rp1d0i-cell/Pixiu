...
> 创建：2026-03-09
> 前置依赖：`v2_architecture_overview.md`、`v2_interface_contracts.md`
...
仓库如果存在 `@pytest.mark.asyncio`，则必须显式引入并配置 `pytest-asyncio`。

目标状态：
- dev/test 依赖中包含 `pytest-asyncio`
- 在 pytest 配置中注册 `asyncio` marker
...
```bash
PYTHONPATH=. pytest -q tests -m "smoke or unit"
```

### 本地集成

```bash
PYTHONPATH=. pytest -q tests -m "integration and not live and not e2e"
```

### Live 数据联通

```bash
PYTHONPATH=. pytest -q tests -m live
```

### 端到端闭环

```bash
PYTHONPATH=. pytest -q tests -m e2e
```

在 marker 体系和 pytest 配置完成前，不应将 `pytest` 裸命令宣传为项目的稳定测试入口。
...
- `v2_interface_contracts.md` 必须映射到 schema 测试。
- `v2_stage3_prefilter.md` 必须映射到 prefilter 测试。
- `v2_stage4_execution.md` 必须映射到 execution 测试。
- `v2_stage5_judgment.md` 必须映射到 judgment 测试。
- `v2_terminal_dashboard.md` 的 CLI/API 最小实现，必须至少有 import 和接口 smoke 测试。
