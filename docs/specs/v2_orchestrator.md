...
> 文件位置：`src/core/orchestrator.py`（重写）
...
```python
# src/core/orchestrator.py 的入口函数
...
# src/core/config.py
MAX_ROUNDS = 100
ACTIVE_ISLANDS = ["momentum", "northbound", "valuation", "volatility"]
REPORT_EVERY_N_ROUNDS = 5           # 每 5 轮强制生成 CIO 报告
STAGE3_TOP_K = 5                    # Stage 3 最多放行候选数
