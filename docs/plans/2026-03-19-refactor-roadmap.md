...
Purpose: Define the test-pipeline pre-gate plus the next three standalone architecture refactors, their ordering, and their dependency boundaries.
...
因此推荐先完成一个测试体系前置 gate，再进入三条可独立推进的 epic。

---

## Recommended Sequence

### Pre-Gate — Test Pipeline Refactor

最先做。

原因：

- 当前测试层已经成为 B/C 重构的前置约束
- 如果 marker、fixture、global state 和 contract tests 不先收口，后续边界重构会被假红灯和假绿灯拖住
- 这轮的目标不是加覆盖率，而是让测试层重新成为可信的工程基础设施

见：

- `docs/plans/2026-03-20-test-pipeline-refactor-design.md`
- `docs/plans/2026-03-20-test-pipeline-refactor-implementation.md`

### Epic A — Data Capability Platform

在 Test Pipeline Refactor 之后继续推进。
...
- Test Pipeline Refactor 是 Epic B/C 的前置 gate
- Epic A 不依赖 Epic B/C
