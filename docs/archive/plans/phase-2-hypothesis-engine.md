...
设计文档 `docs/design/21_stage-2-hypothesis-expansion.md` Section 8 指出三个缺口：
...
|------|------|
| 优先级 | P0 — 所有子空间依赖 |
| 设计文档 | `docs/design/17_failure-constraint.md` |
...
- `src/agents/judgment/` — verdict 生成后自动提取 FailureConstraint
...
|------|------|
| 优先级 | P0 — 填充空壳节点 |
| 设计文档 | `docs/design/18_synthesis-agent.md` |
| 写集 | `src/agents/synthesis.py`, `src/core/orchestrator/` |
| 依赖 | 无（可与 2.1 并行） |
...
- `src/core/orchestrator/` — synthesis_node 调用 SynthesisAgent
...
|------|------|
| 优先级 | P2（降级，核心已完成） |
| 设计文档 | `docs/design/21_stage-2-hypothesis-expansion.md` §9 |
...
|------|------|
| 优先级 | P1（仍需做，纯符号路径价值高） |
| 设计文档 | `docs/design/21_stage-2-hypothesis-expansion.md` §10 |
...
|------|------|
| 优先级 | P1 |
| 设计文档 | `docs/design/21_stage-2-hypothesis-expansion.md` §11 |
